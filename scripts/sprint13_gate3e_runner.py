"""Sprint13 Gate3E Economic Evidence Runner.

Builds and persists candidate-scoped economic evidence coverage for all 58 Sprint13 candidates under Gate3E.
"""
...
def build_gate3e_coverage(run_id):
    ...
    with client.get_connection() as conn, conn.cursor(
        ...
        require(
            len(verified_items) + len(unevaluable_items) == 46,
            "Eligible dispositions must partition into VERIFIED + LEGITIMATELY_UNEVALUABLE == 46",
        )
        ...
        payload = {
            ...
            "threshold": THRESHOLD,
            "status": (
                "SPRINT13_GATE3E_GO_READY"
                if len(qualified_list) >= 3
                else "SPRINT13_GATE3E_GO_NOT_READY"
            ),
...
def canonical_payload_3e(payload: dict) -> str:
    result = dict(payload)
...
def payload_hash_3e(payload: dict) -> str:
    return hashlib.sha256(canonical_payload_3e(payload).encode("utf-8")).hexdigest()
...
def persist_gate3e():
    ...
    print(f"Persisted Gate3E run: {run_id}")
